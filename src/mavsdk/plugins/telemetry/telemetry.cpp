...
bool operator==(const Telemetry::Ina219& lhs, const Telemetry::Ina219& rhs)
{
    return
        (rhs.time == lhs.time) &&
        ((std::isnan(rhs.left_voltage) && std::isnan(lhs.left_voltage)) || rhs.left_voltage == lhs.left_voltage) &&
        ((std::isnan(rhs.left_current) && std::isnan(lhs.left_current)) || rhs.left_current == lhs.left_current) &&
        ((std::isnan(rhs.left_power) && std::isnan(lhs.left_power)) || rhs.left_power == lhs.left_power) &&
        ((std::isnan(rhs.right_voltage) && std::isnan(lhs.right_voltage)) || rhs.right_voltage == lhs.right_voltage) &&
        ((std::isnan(rhs.right_current) && std::isnan(lhs.right_current)) || rhs.right_current == lhs.right_current) &&
        ((std::isnan(rhs.right_power) && std::isnan(lhs.right_power)) || rhs.right_power == lhs.right_power);
}
...
std::ostream& operator<<(std::ostream& str, Telemetry::Ina219 const& ina219)
{
    ...
    str << "    left_voltage: " << ina219.left_voltage << '\n';
    str << "    left_current: " << ina219.left_current << '\n';
    str << "    left_power: " << ina219.left_power << '\n';
    str << "    right_voltage: " << ina219.right_voltage << '\n';
    str << "    right_current: " << ina219.right_current << '\n';
    str << "    right_power: " << ina219.right_power << '\n';
    str << '}';
    return str;
}
